...
The Agentic Harness is a layered execution framework for orchestrating AI agents, skills, and multi-agent coordination. It consists of six layers, each with a distinct responsibility.
...
│                    Lifecycle Layer                       │
│              (task.ts, hooks.ts)                         │
│         Task management + event-driven hooks             │
├─────────────────────────────────────────────────────────┤
│                  Multi-Agent Layer                       │
│           (fork.ts, swarm.ts, coordinator.ts)            │
│     Process forking, swarm coordination, task routing    │
├─────────────────────────────────────────────────────────┤
│                    Context Layer                          │
│                      (context.ts)                        │
│         Context building, injection, snapshots           │
├─────────────────────────────────────────────────────────┤
│                     Tools Layer                           │
│                      (executor.ts)                        │
│           Bash command execution engine                  │
├─────────────────────────────────────────────────────────┤
│                    Skills Layer                           │
│                   (skill-runner.ts)                      │
...
│                     Memory Layer                          │
│                     (memory.ts)                           │
│         LLM-powered memory with semantic search          │
└─────────────────────────────────────────────────────────┘
```

### Memory Layer (`memory.ts`)

Persistent memory with LLM-powered extraction. Stores memories as JSONL files at `$CWD/.gstack-harness/memory/`.
...
- `project-context` — Architecture decisions, conventions, design patterns
- `user-preferences` — User's workflow preferences, tools they prefer

**Key Functions:**
- `save(agentId, memoryType, content)` — Save with optional background LLM extraction
- `recall(agentId, query)` — Search memories by raw content or extracted summary/tags
- `extract(agentId, rawContent, memoryType)` — Call LLM to structure raw content

**Storage Format:**
```
$CWD/.gstack-harness/memory/<type>.jsonl
```

Each entry is newline-delimited JSON with `id`, `agent_id`, `memory_type`, `content`, `extracted_content`, `created_at`, `updated_at`.

### Skills Layer (`skill-runner.ts`)

Loads and executes Markdown-based skills with `## Workflow` and `## Execution` sections.

**Skill File Resolution Order:**
1. `harness/skills/<name>.md`
2. `harness/skills/<name>/SKILL.md`
3. `harness/skills/<name.toLowerCase()>.md`
4. `harness/skills/<name.toLowerCase()>/SKILL.md`

**Execution Model:**
- Parses frontmatter (`name`, `description`, `triggers`)
- Extracts `## Workflow` (human-readable description)
- Extracts `## Execution` (bash commands in ```bash blocks)
- Runs commands sequentially with `ARG_*` environment variables
- Supports `--arg key=value` CLI arguments

**CLI Usage:**
```bash
bun run src/runtime/skill-runner.ts --skill <name> [--arg key=value]
bun run src/runtime/skill-runner.ts --list  # Show available skills
```

### Tools Layer (`executor.ts`)

Step execution engine using Bun.spawn with full stdout/stderr capture.

**Core API:**
```typescript
executeBash(command: string): Promise<StepResult>
executeBashWithOptions(command: string, options?: ExecuteOptions): Promise<StepResult & { success: boolean }>
```
...
```typescript
{
  ...
  stopOnError?: boolean // Default: true - halt on non-zero exit
}
```

**StepExecutor Class:**
- Status tracking (`pending` → `running` → `success`/`failed`)
- `executeSteps(commands, options)` — Run multiple commands with optional halt on error

### Context Layer (`context.ts`)

Context building, skill injection, and snapshot management.
...
**Storage:** `$CWD/.gstack/context/` for snapshots, `$CWD/.gstack/context/current.json` for active context

### Multi-Agent Layer (`fork.ts`, `swarm.ts`, `coordinator.ts`)

**fork.ts — Real Process Forking**

Creates independent child processes via Bun.spawn with IPC communication.
...
Child process sets `GSTACK_FORK_MODE=child` environment variable and re-executes skill-runner.
...
| `hierarchical` | Master agent spawns child agents recursively |
...
// Returns: { success, totalDuration, agents[], aggregated{} }
```

Aggregated result includes `totalAgents`, `successfulAgents`, `failedAgents`, `successRate`, `pids[]`.

**coordinator.ts — Task Scheduling and Agent Management**

Register agents, schedule tasks, monitor health, and reschedule failed tasks.

```typescript
const coordinator = new Coordinator();
coordinator.register({ id: 'agent-1', name: 'Worker', capabilities: ['coding'], maxConcurrentTasks: 3, priority: 8 });
await coordinator.schedule({ type: 'code-review', priority: 5, requiredCapabilities: ['review'], payload: {}, maxRetries: 3, timeoutMs: 300000 });
```

**Agent State Tracking:**
- Status: `idle` | `busy` | `failing` | `terminated`
- Health score (decrements on failure, reschedules task if agent fails)
- Current tasks / max concurrent tasks
- Total completed / failed counts

**Storage:** `$CWD/.gstack-harness/coordinator/agents/` and `$CWD/.gstack-harness/coordinator/scheduled/`

### Lifecycle Layer (`task.ts`, `hooks.ts`)

**task.ts — Task Lifecycle Management**

Task creation, execution, and cancellation with step-level tracking.
...
**Step Execution:**
- Each bash command from skill becomes a step
- Steps tracked individually: `pending` → `running` → `success`/`failed`
- Each step records `stdout`, `stderr`, `exit_code`, `started_at`, `completed_at`
- Task completes when all steps succeed (or first failure if `stopOnError`)

**Storage:** `$CWD/.gstack-harness/tasks/<task_id>.json`

**hooks.ts — Event-Driven Hook System**

Registry for event-driven hooks with priority-based execution.
...
interface HookResult {
  allowed?: boolean;   // Permission for continuation
  reason?: string;     // Explanation if blocked
  blocked?: boolean;   // Stop the hook chain
  data?: unknown;      // Data to pass through
}
```

**HookRegistry API:**
```typescript
registry.register(name, hookFn, priority: number)  // Lower priority = earlier execution
registry.unregister(name)
registry.trigger(name, data)  // Returns HookResult or HookResult[]
registry.hasHooks(name)
```

**Blocking Behavior:**
- If a hook returns `blocked: true` or `allowed: false`, the chain stops
- Failed hooks (exceptions) also halt execution

**Storage:** `$CWD/.gstack-harness/hooks.json` (serialized hook names and priorities)

### Design Principles

1. **Separation of concerns** — Each layer has a single responsibility (memory, skills, tools, context, multi-agent, lifecycle)
2. **Unix philosophy** — Small tools that do one thing well; compose via IPC and shared storage
3. **Event-driven** — Lifecycle hooks allow interception at key points without modifying core logic
4. **Process isolation** — Forked agents run in separate processes, not threads
5. **Persistence by default** — All state written to disk in `$CWD/.gstack-harness/` for crash recovery
