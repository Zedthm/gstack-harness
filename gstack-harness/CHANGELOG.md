...
## **Agentic Harness Framework v2 — 6-layer execution architecture**

This release documents and formalizes the Agentic Harness Framework as a first-class component of gstack. The framework provides layered execution for orchestrating AI agents, skills, and multi-agent coordination.

### Architecture Changes

Added comprehensive documentation for the 6-layer framework:
...
- **ARCHITECTURE.md** — Added "Agentic Harness Framework (v2)" section with complete layer descriptions, data flow diagrams, and design principles
- **HARNESS.md** — New usage guide covering all six layers, CLI reference, storage layout, and full multi-agent workflow examples

### Layer Details

**Memory Layer (`memory.ts`):**
- Persistent JSONL storage at `$CWD/.gstack-harness/memory/`
- Four memory types: `skill-patterns`, `error-patterns`, `project-context`, `user-preferences`
- LLM-powered extraction with background async processing
- `save()`, `recall()`, `extract()` API with `openai-api-key` auth resolution

**Skills Layer (`skill-runner.ts`):**
- Markdown-based skills with `## Workflow` + `## Execution` sections
- Four search paths for skill resolution
- Environment variable injection (`ARG_*`) for CLI args
- `findSkillFile()`, `parseSkill()`, `extractBashCommands()` exports

**Tools Layer (`executor.ts`):**
- `executeBash()` and `executeBashWithOptions()` using `Bun.spawn`
- `StepExecutor` class with status tracking (`pending` → `running` → `success`/`failed`)
- `executeSteps()` for sequential command execution with optional halt on error

**Context Layer (`context.ts`):**
- Context composition: recent_memories, active_task, skill_metadata, user_preferences, injected_skills
- Snapshot/restore with `createSnapshot()`, `saveSnapshot()`, `loadSnapshot()`, `restoreContext()`
- CLI commands: `--build`, `--inject`, `--snapshot`, `--restore`

**Multi-Agent Layer:**
- `fork.ts` — Real process forking via `Bun.spawn` with IPC, `GSTACK_FORK_MODE` environment variable
- `swarm.ts` — Three coordination modes: `sequential`, `parallel`, `hierarchical`
- `coordinator.ts` — `Coordinator` class with agent registration, task scheduling, health monitoring, rescheduling

**Lifecycle Layer:**
- `task.ts` — Task creation, step execution, cancellation with step-level tracking at `$CWD/.gstack-harness/tasks/`
- `hooks.ts` — `HookRegistry` with priority-ordered execution, blocking semantics, event types: `PreToolUse`, `PostToolUse`, `OnAgentStart`, `OnAgentEnd`, `OnError`
...
5. Unix philosophy — small composable tools layered via IPC and shared storage
